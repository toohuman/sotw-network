    ...
    for n, noise in enumerate(noise_values):
        for a, agents in enumerate(agents_set):
            for c, con in enumerate(connectivity_values):
                ...
                print("Average Error: {} states | {:.2f} con | {:.2f} noise".format(states, con, noise))
                for e, er in enumerate(evidence_rates):
                    print("   [{:.2f} er]:  ".format(er), end="")
                    for k, knn in enumerate(knn_values):
                        print("[{} knn]: {:.3f}".format(knn, results[e][k]), end=" ")
                    print("")
                ...
                for e, er in enumerate(evidence_rates):
                    ...
                    ax = sns.lineplot(x = knn_values, y = results[e], linewidth = 2)
                ...
                plt.tight_layout()
                plt.savefig("../../results/graphs/sotw-network/loss_WS_{}_states_{}_agents_{:.2f}_con_{:.2f}_noise_shaded.pdf".format(states, agents, con, noise))
                plt.clf()