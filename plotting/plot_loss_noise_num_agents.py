...
connectivity_values = [0.01, 0.1, 1.0]
    ...
    for e, er in enumerate(reversed(evidence_rates)):
        for n, noise in enumerate(noise_values):
            ...
            for c, con in enumerate(connectivity_values):
                ...
                print(loss_results)
                cmap = sns.cm.rocket
                ax = sns.lineplot(agents_set, loss_results[c], linewidth = 2)
                plt.axhline(noise, color="red", linestyle="dotted")
                plt.xlabel("Agents")
                plt.ylabel("Average Loss")
                plt.ylim(-0.01, 0.55)
                plt.title("Average loss | {} states, {} con, {} er, {} noise".format(states * states, connectivity_strings[c], er, noise))
                # plt.legend()
                # plt.show()
                plt.savefig("../../results/graphs/sotw-network/loss_{}_states_{}_con_{:.2f}_er_{:.2f}_noise.pdf".format(states, connectivity_strings[c], er, noise, con))
                plt.clf()