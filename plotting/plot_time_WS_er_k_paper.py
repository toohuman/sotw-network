    ...
    for n, noise in enumerate(noise_values):
        for a, agents in enumerate(agents_set):
            for c, con in enumerate(connectivity_values):
                ...
                results = np.array([[0.0 for x in knn_values] for y in evidence_rates])
                lowers = np.array([[0.0 for x in knn_values] for y in evidence_rates])
                uppers = np.array([[0.0 for x in knn_values] for y in evidence_rates])
                data = None
                ...
                for e, er in enumerate(evidence_rates):
                    for k, knn in enumerate(knn_values):
                        ...
                        time_diff = 100 # time steps
                        max_iterations = 10_000
                        time_tracker = [0 for _ in range(100)]
                        convergence_time = [max_iterations for _ in range(100)]

                        for i, tests in enumerate(data):
                            # i is the iteration count
                            if i == 0:
                                continue
                            for j, run in enumerate(tests):
                                if time_tracker[j] == -1:
                                    continue
                                # j is the run number
                                if run[0] == data[i-1][j][0]:
                                    time_tracker[j] += 1
                                else:
                                    time_tracker[j] = 0
                                if time_tracker[j] >= time_diff:
                                    convergence_time[j] = i - time_diff
                                    time_tracker[j] = -1

                        # print(convergence_time)

                        data = sorted(convergence_time)
                        lowers[e][k] = data[PERC_LOWER - 1]
                        uppers[e][k] = data[PERC_UPPER - 1]
                        results[e][k] = np.average(data)

                        # print(lowers[e][k], results[e][k], uppers[e][k])

                print("Convergence Time: {} states | {:.2f} con | {:.2f} noise".format(states, con, noise))
                for e, er in enumerate(evidence_rates):
                    print("   [{:.2f} er]:  ".format(er), end="")
                    for k, knn in enumerate(knn_values):
                        print("[{} knn]: {:.1f}".format(knn, results[e][k]), end=" ")
                    print("")
                ...
                for e, er in enumerate(evidence_rates):
                    ...
                    ax = sns.lineplot(x = knn_values, y = results[e], linewidth = 2, label=r'$r$ = {}'.format(er))
                    plt.fill_between(knn_values, lowers[e], uppers[e], facecolor=sns.color_palette()[e], edgecolor="none", alpha=0.3, antialiased=True)
                # plt.axhline(noise, color="red", linestyle="dotted", linewidth = 2)
                plt.xlabel(r'Nearest neighbours $k$')
                plt.ylabel(r'Convergence time $t$')
                plt.xticks(knn_values, knn_strings)
                plt.ylim(0, 10_500)
                ...
                plt.tight_layout()
                plt.savefig("../../results/graphs/sotw-network/time_WS_{}_states_{}_agents_{:.2f}_con_{:.2f}_noise_shaded.pdf".format(states, agents, con, noise))
                plt.clf()